...
repeat: existing policies, workspaces, routes, and policy files are kept.
  ...
  5. Attach lore project memory (LORE_CONTEXT, default ``lore-<workspace>`` when
     ...
     ``claude/mcp.json``, ``mcp__lore__<tool>`` allow rules in its settings.json
     for every tool the installed lore reports (``lore mcp --list-tools``) minus
     LORE_DENY_TOOLS (default lore_remember), and a marker-fenced lore section
     in the workspace AGENTS.md that re-runs refresh. Applies to a pre-existing
     policy too; allow/deny are reconciled, nothing else is removed.
...
Inputs from the environment (install.sh exports them):
  ...
  LORE_CONTEXT, LORE_REMOTE, LORE_DENY_TOOLS, POLICY_TEMPLATE, WORKSPACE_TEMPLATE,
...
# The lore allow list is not written down here: it comes from the installed
# binary (`lore mcp --list-tools`) at route time, so a new agent always gets
# every tool its lore build exposes. LORE_DENY_TOOLS (conf; default
# lore_remember) names the ones client-facing channels must not trigger:
# pinning facts into the client's memory is an operator action, done from the
# admin DM route. LORE_TOOLS_FALLBACK is only for a lore too old to have
# --list-tools; keep it equal to lore's MCP_TOOLS table when you touch it.
LORE_DENY_DEFAULT = "lore_remember"
LORE_TOOLS_FALLBACK = (
    {"name": "lore_grep", "writes": False},
    {"name": "lore_read", "writes": False},
    {"name": "lore_recall", "writes": False},
    {"name": "lore_sync_now", "writes": False},
    {"name": "lore_remember", "writes": True},
    {"name": "lore_sow_add", "writes": True},
    {"name": "lore_doc_add", "writes": True},
    {"name": "lore_work_add", "writes": True},
    {"name": "lore_work_promote", "writes": True},
    {"name": "lore_work_move", "writes": True},
    {"name": "lore_work_set", "writes": True},
)
LORE_BEGIN = "<!-- enso-agent-bootstrap:lore begin (regenerated by install.sh; edit outside the markers) -->"
LORE_END = "<!-- enso-agent-bootstrap:lore end -->"
LORE_SECTION = """
## Project memory (lore)

The approved `lore` MCP server holds this project's memory (context `__LORE_CONTEXT__`):
synced Slack history, GitHub and Jira activity, meeting notes, attached
documents, statements of work, the lore work tracker, and derived requests,
decisions, and pinned facts. The `lore-mcp` skill explains how to query it and
when each write is appropriate.

Tools you can call here: __LORE_ALLOWED__.
__LORE_DENIED_LINE__
...
- The lore tracker in `lore_recall` is authoritative for what is open, in
  progress, blocked, or done; Slack and meetings are evidence, not decisions.
...
- Every write records you as the actor with the reason you give; act on a
  person's explicit ask or unambiguous evidence, never on a hunch.
"""
# The section as written by install.sh before the markers existed (up to the
# "administrator will pin it" closing line), so a re-run can replace it.
LORE_LEGACY_RE = re.compile(
    r"## Project memory \(lore\)\n\nThe approved `lore` MCP server holds this project's memory.*?"
    r"an administrator will pin it\.\n",
    re.S,
)
LORE_DENIED_LINE = (
    "You cannot call __LORE_DENIED__ from here (denied by policy); if something\n"
    "should be pinned or attached that way, say so and an administrator will do it."
)
...
def wire_lore(paths_root: str, policy_dir: str, workspace: str, context: str) -> list[str]:
    ...
    allowed, denied = lore_tool_split(lore_bin)
    ...
    changed = False
    for tool in allowed:
        rule = f"mcp__lore__{tool}"
        if rule in deny:  # the conf stopped denying it: the operator's explicit input wins
    ...
    if changed:
        ...
        info(f"lore: settings.json now allows {', '.join(allowed)}" + (f" and denies {', '.join(denied)}" if denied else ""))
    else:
        info(f"lore: settings.json already allows {', '.join(allowed)}" + (f" and denies {', '.join(denied)}" if denied else ""))

    # 3. Tell the workspace what it has. The section sits between markers so a
    # re-run refreshes the tool list; a hand-written lore section (no markers)
    # is left alone.
    ...
    if os.path.isfile(agents) and not os.path.islink(agents):
        with open(agents, encoding="utf-8") as fh:
            text = fh.read()
        section = LORE_BEGIN + lore_section(context, allowed, denied) + LORE_END + "\n"
        if LORE_BEGIN in text and LORE_END in text:
            head, rest = text.split(LORE_BEGIN, 1)
            _, tail = rest.split(LORE_END + "\n", 1) if LORE_END + "\n" in rest else rest.split(LORE_END, 1)
            new_text = head + section + tail
            if new_text != text:
                with open(agents, "w", encoding="utf-8") as fh:
                    fh.write(new_text)
                _commit(paths_root, agents, f"bootstrap: refresh lore tool list in {workspace}")
                info("lore: refreshed the project-memory section of the workspace AGENTS.md")
        elif LORE_LEGACY_RE.search(text):
            # A section written by an earlier install.sh (before the markers), with
            # the four-tool list: replace it in place so re-running fixes old VMs.
            new_text = LORE_LEGACY_RE.sub(lambda _m: section.rstrip("\n") + "\n", text, count=1)
            with open(agents, "w", encoding="utf-8") as fh:
                fh.write(new_text)
            _commit(paths_root, agents, f"bootstrap: refresh lore tool list in {workspace}")
            info("lore: replaced the pre-marker project-memory section of the workspace AGENTS.md")
        elif "lore" in text.lower():
            info("lore: workspace AGENTS.md already mentions lore without bootstrap markers; left alone")
        else:
    ...
    notes.append(f"             tools allowed: {', '.join(allowed)}" + (f"; denied: {', '.join(denied)}" if denied else ""))
    return notes


def lore_tools(lore_bin: str) -> list[dict]:
    """The tool table of the installed lore (`lore mcp --list-tools`), or the fallback."""
    try:
        out = subprocess.run(
            [lore_bin, "mcp", "--list-tools"], capture_output=True, text=True, timeout=60, check=True,
            env={**os.environ, "HOME": os.path.expanduser("~")},
        ).stdout
        tools = json.loads(out)
        names = [t["name"] for t in tools if isinstance(t, dict) and str(t.get("name", "")).startswith("lore_")]
        if names:
            return [t for t in tools if t["name"] in names]
        warn("lore mcp --list-tools printed no tools; using the built-in fallback list")
    except (subprocess.CalledProcessError, subprocess.TimeoutExpired, OSError, ValueError) as exc:
        warn(f"lore mcp --list-tools unavailable ({type(exc).__name__}); this lore predates it. "
             f"Using the built-in fallback list; rebuild lore (install.sh --tools-only) to be sure it is complete")
    return [dict(t) for t in LORE_TOOLS_FALLBACK]


def lore_tool_split(lore_bin: str) -> tuple[list[str], list[str]]:
    """(allowed, denied) tool names for the restricted policy, from the installed lore and LORE_DENY_TOOLS."""
    names = [t["name"] for t in lore_tools(lore_bin)]
    wanted_deny = split_list(env("LORE_DENY_TOOLS", LORE_DENY_DEFAULT))
    wanted_deny = [d if d.startswith("lore_") else f"lore_{d}" for d in wanted_deny]
    unknown = [d for d in wanted_deny if d not in names]
    if unknown:
        warn(f"LORE_DENY_TOOLS names tools this lore does not expose: {', '.join(unknown)} (denied anyway)")
    denied = [n for n in names if n in wanted_deny] + unknown
    allowed = [n for n in names if n not in wanted_deny]
    return allowed, denied


def lore_section(context: str, allowed: list[str], denied: list[str]) -> str:
    denied_line = LORE_DENIED_LINE.replace("__LORE_DENIED__", ", ".join(f"`{d}`" for d in denied)) if denied else ""
    return (
        LORE_SECTION.replace("__LORE_CONTEXT__", context)
        .replace("__LORE_ALLOWED__", ", ".join(f"`{a}`" for a in allowed))
        .replace("__LORE_DENIED_LINE__\n", denied_line + "\n" if denied_line else "")
    )
